...
| Skills `load_skill` | `skill` 工具，31 个全部已挂载（见 §3） |
...
- 挂载方式：`C:\Users\dlzhi\.dsh\skills\` 下为 31 个目录联接，指向本项目的 `skills\`（用户级，所有 DSH 会话可见）。
- 已修复 6 个不兼容项：
...
- 校验脚本：`C:\Users\dlzhi\.dsh\check-skills.mjs`（用 DSH 同款解析器全量校验 31 个 frontmatter）。
...
环境已就绪：31 个 skills 可用（skill 工具）；4 个 MCP 已配（mcp__memory__* / mcp__sequential-thinking__* / mcp__alchemy__* / mcp__qdrant__*）；node v24 可跑 scripts/*.js。
